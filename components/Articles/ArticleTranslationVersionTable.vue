<template>
  <div class="card mb-0">
      ...
      <template #cell(actions)="{item: {id}}">
        <b-dd
          ...
          <b-dd-item
            ...
            :to="`/user/translation-versions/${id}`">
            Review
          </b-dd-item>
    ...
    <div class="card-footer d-flex align-items-center">
      <custom-pager
        v-model="page"
        :rows="translationVersions.length"
        :per-page="per"
        class="m-0" />
      <div class="ml-auto">
        Total Translation Versions
        <md-icon>remove</md-icon>
        <strong>{{ translationVersions.length }}</strong>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    paginatedItems() {
      ...
      let items = this.translationVersions.slice(start, start + this.per)
